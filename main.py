...
class Net(nn.Module):
    def __init__(self):
        ...
        self.fc1 = nn.Linear(320, 50)  # 第一层全连接层
        self.fc2 = nn.Linear(50, 10)  # 第二层全连接层
...
def train(epoch):
    ...
    for batch_index, (data, target) in enumerate(train_loader):
        optimizer.zero_grad()
        output = network(data)
        loss = F.nll_loss(output, target)  # 损失函数：负对数似然损失
        loss.backward()  # 计算输出output与target的误差回传
        optimizer.step()
...
# 测试
def test():
    network.eval()  # 在评估模式下设置模型，适用于Dropout图层
    test_loss = 0
    correct = 0
    with torch.no_grad():
